//! [`WarcraftObjectId`]: the object aggregate's identity — a *domain* model of
//! identity, deliberately distinct from the game's in-memory `Identifier`
//! primitive (`warcraft-primitives`): here an id is a static, already-known
//! string that only the crate's own authoritative data may mint.
