//! Unit catalog query: assembles the sorted, variant-collapsed list of units
//! for a race/mode/kind/search view. An `ApplicationService` reading through
//! `WarcraftApi`; it returns [`CatalogEntry`] read models. The query knobs
//! ([`SearchField`], [`CatalogVisibility`]) are domain value objects.
...
use crate::application::command_catalog::CommandCatalog;
use crate::application::unit::variant::registry as variant_registry;
use crate::domain::catalog::{CatalogVisibility, SearchField};
...
impl CatalogEntry {
    ...
    fn canonical_entry(unit_id: WarcraftObjectId) -> Option<Self> {
        ...
        let effective_kind = unit_meta.effective_kind();
        let entry = Self {
            unit_id,
            warcraft_object,
...
impl UnitCatalog {
    /// The first unit a `race`/`mode` list would show under the default,
    /// curated visibility — the natural default selection for that race.
    pub fn default_unit_id_for(race: Race, mode: UnitMode) -> Option<WarcraftObjectId> {
        Self::entries_for(
            Some(race),
            Some(mode),
            None,
    ...
    pub fn entries_for(
        ...
        let candidates: Vec<Candidate> = WarcraftApi::default()
            ...
            .filter_map(|(object_id, warcraft_object)| {
                ...
                let passes_mode = match mode_filter {
                    Some(mode) => unit_meta.passes_filter(mode),
                    None => {
                        unit_meta.passes_filter(UnitMode::Melee)
                            || unit_meta.passes_filter(UnitMode::Campaign)
                    }
                };
                if !passes_mode {
                    return None;
        ...
        for entry in visible_entries {
            ...
            let canonical_lookup = variant_registry::canonical(entry.unit_id);
        ...
        entries.sort_by(|left_entry, right_entry| {
            ...
            let left_priority = if is_search {
                ...
                left_kind.search_sort_priority(left_campaign)
            } else {
                left_entry.unit_kind.category_priority()
            };
            let right_priority = if is_search {
                ...
                right_kind.search_sort_priority(right_campaign)
            } else {
                right_entry.unit_kind.category_priority()
            };
            left_priority
                .cmp(&right_priority)
...
// DDD roles: the catalog query is an application service; each returned entry
// is a read model.
impl ddd::ReadModel for CatalogEntry {}
impl ddd::Layered for UnitCatalog {
