//! Variant registry assembly and queries. Builds the merged variant groups from
//! the game data (union-find over tier/swap/hero-name evidence) over the static
//! database, and exposes them through crate-internal query functions that the
//! `UnitApi` and `AbilityApi` variant edges wrap.
...
use crate::{
    TIERED_UNIT_GROUPS, UNIT_UPGRADE_SWAPS, UnitKind, WarcraftObjectId, WarcraftObjectMeta,
};
...
impl VariantGraphBuilder {
    ...
    fn into_registry(self) -> VariantRegistry {
        ...
        for component in members_by_root.into_values() {
            ...
            if members.len() >= 2 {
                let group = VariantGroup::new(members);
                groups.push(group);
            }
        }
...
/// The variant group a unit belongs to, or `None` when the unit stands alone
/// (the common case).
pub(crate) fn variant_group(unit_id: WarcraftObjectId) -> Option<&'static VariantGroup> {
...
/// The strongest sibling of a unit (itself when it already is the strongest),
/// or `None` when the unit is not part of any group.
pub(crate) fn canonical(unit_id: WarcraftObjectId) -> Option<WarcraftObjectId> {
...
/// canonical sibling.
pub(crate) fn is_hidden_variant(unit_id: WarcraftObjectId) -> bool {
    VARIANT_REGISTRY.hidden_members.contains(&unit_id)
}

/// The other tier abilities that must receive the same hotkey/position edit as
/// `ability_id` — its same-mechanic, same-cell counterparts on the other tiers
/// of its variant group, restricted to abilities with a *different* id (same-id
/// tiers already share one binding). Empty for almost every ability; non-empty
/// only for different-id tiers like the Carrion Beetle's Burrow (`Abu3` →
/// `Abu2`).
pub(crate) fn fanout_siblings(ability_id: WarcraftObjectId) -> &'static [WarcraftObjectId] {
...
mod tests {
    ...
    fn leveled_summon_tiers_collapse_to_strongest() {
        let group =
            variant_group(WarcraftObjectId::new("osw1")).expect("osw1 belongs to a variant group");
        ...
        assert_eq!(
            canonical(WarcraftObjectId::new("osw1")),
            Some(WarcraftObjectId::new("osw3"))
        );
        assert_eq!(
            canonical(WarcraftObjectId::new("osw2")),
            Some(WarcraftObjectId::new("osw3"))
        );
        assert!(is_hidden_variant(WarcraftObjectId::new("osw1")));
        assert!(is_hidden_variant(WarcraftObjectId::new("osw2")));
        assert!(!is_hidden_variant(WarcraftObjectId::new("osw3")));
    }
    ...
    fn overlapping_summon_chains_union_merge() {
        let quillbeast =
            variant_group(WarcraftObjectId::new("nqb1")).expect("nqb1 belongs to a group");
        assert_eq!(
            quillbeast.members(),
            [
        ...
        assert_eq!(
            canonical(WarcraftObjectId::new("nqb3")),
            Some(WarcraftObjectId::new("nqb4"))
        );

        let spiderling =
            variant_group(WarcraftObjectId::new("osp1")).expect("osp1 belongs to a group");
        assert_eq!(
            spiderling.members(),
            [
    ...
    fn headhunter_berserker_swap_canonical_is_berserker() {
        assert_eq!(
            canonical(WarcraftObjectId::new("ohun")),
            Some(WarcraftObjectId::new("otbk"))
        );
        assert!(is_hidden_variant(WarcraftObjectId::new("ohun")));
        assert!(!is_hidden_variant(WarcraftObjectId::new("otbk")));
    }
    ...
    fn siege_engine_barrage_swap_canonical_is_upgraded() {
        assert_eq!(
            canonical(WarcraftObjectId::new("hmtt")),
            Some(WarcraftObjectId::new("hrtt"))
        );
        assert!(is_hidden_variant(WarcraftObjectId::new("hmtt")));
        assert!(!is_hidden_variant(WarcraftObjectId::new("hrtt")));
    }
    ...
    fn curated_carrion_beetle_group_collapses() {
        let group =
            variant_group(WarcraftObjectId::new("ucs2")).expect("ucs2 is a curated tier member");
        assert_eq!(
            group.members(),
            [
                WarcraftObjectId::new("ucs1"),
        ...
        assert!(is_hidden_variant(WarcraftObjectId::new("ucs1")));
        assert!(is_hidden_variant(WarcraftObjectId::new("ucs2")));
        assert!(!is_hidden_variant(WarcraftObjectId::new("ucs3")));

        // The burrowed beetle forms collapse the same way.
        let burrowed =
            variant_group(WarcraftObjectId::new("ucsB")).expect("ucsB is a curated tier member");
        assert_eq!(
            burrowed.members(),
            [WarcraftObjectId::new("ucsB"), WarcraftObjectId::new("ucsC")]
        );
        assert_eq!(burrowed.canonical(), WarcraftObjectId::new("ucsC"));
        assert!(is_hidden_variant(WarcraftObjectId::new("ucsB")));
        assert!(!is_hidden_variant(WarcraftObjectId::new("ucsC")));
    }
    ...
    fn curated_clockwerk_goblin_group_collapses() {
        let group =
            variant_group(WarcraftObjectId::new("ncg1")).expect("ncg1 is a curated tier member");
        assert_eq!(
            group.members(),
            [
                WarcraftObjectId::new("ncg1"),
        ...
        assert!(is_hidden_variant(WarcraftObjectId::new("ncg1")));
        assert!(is_hidden_variant(WarcraftObjectId::new("ncg2")));
        assert!(is_hidden_variant(WarcraftObjectId::new("ncg3")));
        assert!(!is_hidden_variant(WarcraftObjectId::new("ncgb")));
    }
    ...
    fn clockwerk_goblin_self_destruct_abilities_fan_out() {
        let siblings = fanout_siblings(WarcraftObjectId::new("Asdg"));
        assert!(
        ...
        let from_low = fanout_siblings(WarcraftObjectId::new("Asd2"));
        assert!(
    ...
    fn carrion_beetle_burrow_abilities_fan_out() {
        assert_eq!(
            fanout_siblings(WarcraftObjectId::new("Abu3")),
            [WarcraftObjectId::new("Abu2")]
        );
        assert_eq!(
            fanout_siblings(WarcraftObjectId::new("Abu2")),
    ...
    fn shared_id_tier_abilities_have_no_fan_out() {
        assert!(fanout_siblings(WarcraftObjectId::new("Asal")).is_empty());
        assert!(fanout_siblings(WarcraftObjectId::new("ACct")).is_empty());
    }
    ...
    fn non_unit_false_positives_are_filtered_out() {
        for false_positive_id in ["Rguv", "Reuv"] {
            ...
            assert!(
                variant_group(false_positive_object_id).is_none(),
                "{false_positive_id} must not form a variant group",
            );
            assert!(
                !is_hidden_variant(false_positive_object_id),
    ...
    fn heroes_collapse_to_the_produced_hero() {
        assert_eq!(
            canonical(WarcraftObjectId::new("Nal2")),
            Some(WarcraftObjectId::new("Nalc"))
        );
        assert_eq!(
            canonical(WarcraftObjectId::new("Nal3")),
            Some(WarcraftObjectId::new("Nalc"))
        );
        assert_eq!(
            canonical(WarcraftObjectId::new("Nalm")),
            Some(WarcraftObjectId::new("Nalc"))
        );
        assert!(is_hidden_variant(WarcraftObjectId::new("Nal2")));
        assert!(!is_hidden_variant(WarcraftObjectId::new("Nalc")));

        assert_eq!(
            canonical(WarcraftObjectId::new("Nrob")),
            Some(WarcraftObjectId::new("Ntin"))
        );
        assert!(is_hidden_variant(WarcraftObjectId::new("Nrob")));

        assert_eq!(
            canonical(WarcraftObjectId::new("Huth")),
            Some(WarcraftObjectId::new("Hpal"))
        );
        assert!(is_hidden_variant(WarcraftObjectId::new("Huth")));
        assert!(!is_hidden_variant(WarcraftObjectId::new("Hpal")));
    }
}
