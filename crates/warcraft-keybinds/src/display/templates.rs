...
impl ResolvedTemplate {
    ...
    fn compute_all() -> Vec<Self> {
        let archmage_id = WarcraftApi::default()
            .resolve("Hamg")
            .expect("the Archmage sample unit exists");
        let command_slots: Vec<GridSlotId> = WarcraftApi::default()
            .command_card(archmage_id)
            .filled_slots()
            .collect();
        let research_slots: Vec<GridSlotId> = WarcraftApi::default()
