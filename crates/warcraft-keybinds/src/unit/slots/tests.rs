...
mod unit_slots_tests {
    ...
    fn goblin_lab_command_card_shows_all_three_sell_units() {
        ...
        let has_zeppelin = card
            .filled_slots()
            .any(|slot| slot.id() == crate::test_support::object_id("nzep"));
        let has_shredder = card
            .filled_slots()
            .any(|slot| slot.id() == crate::test_support::object_id("ngir"));
        assert!(
            has_sapper,
    ...
    fn goblin_merchant_command_card_shows_all_eleven_sell_items() {
        ...
        for sell_item_id in sell_item_ids {
            let present = card
                .filled_slots()
                .any(|slot| slot.id() == crate::test_support::object_id(sell_item_id));
            assert!(
                present,
    ...
    fn phoenix_command_card_hides_phoenix_fire() {
        ...
        let has_phoenix_summon = card
            .filled_slots()
            .any(|slot| slot.id() == crate::test_support::object_id("Ahpe"));
        assert!(
            has_phoenix_summon,
    ...
    fn entangled_gold_mine_command_card_hides_load() {
        ...
        let has_unload_all = card
            .filled_slots()
            .any(|slot| slot.id() == crate::test_support::object_id("Adri"));
        assert!(
            has_unload_all,
    ...
    fn forest_troll_high_priest_command_card_contains_exactly_one_abolish_magic() {
        ...
        let abolish_count = card
            .filled_slots()
            .filter(|slot| {
                slot.id() == crate::test_support::object_id("ACdm")
                    || slot.id() == crate::test_support::object_id("ACd2")
            })
            .count();
        assert_eq!(
            abolish_count, 1,
    ...
    fn forest_troll_high_priest_command_card_uses_competitive_abolish_magic() {
        ...
        let has_acdm = card
            .filled_slots()
            .any(|slot| slot.id() == crate::test_support::object_id("ACdm"));
        assert!(
            has_acd2,
    ...
    fn ice_troll_high_priest_command_card_contains_exactly_one_frost_armor() {
        ...
        let frost_armor_count = card
            .filled_slots()
            .filter(|slot| {
                slot.id() == crate::test_support::object_id("ACfu")
                    || slot.id() == crate::test_support::object_id("ACf2")
            })
            .count();
        assert_eq!(
            frost_armor_count, 1,
    ...
    fn ice_troll_high_priest_command_card_uses_competitive_abilities() {
        ...
        let has_acf2 = card
            .filled_slots()
            .any(|slot| slot.id() == crate::test_support::object_id("ACf2"));
        let has_acdm = card
            .filled_slots()
            .any(|slot| slot.id() == crate::test_support::object_id("ACdm"));
        let has_acfu = card
            .filled_slots()
            .any(|slot| slot.id() == crate::test_support::object_id("ACfu"));
        assert!(
            has_acd2,
